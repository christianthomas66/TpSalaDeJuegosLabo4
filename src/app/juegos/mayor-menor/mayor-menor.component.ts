...
export class MayorMenorComponent implements OnInit {
  ...
  comenzarJuego() {
    this.comenzar = true;
    this.carta =
      this.cartaNumber[Math.floor(Math.random() * this.cartaNumber.length)];
  ...
  sigCarta(carta: number, opcion: string) {
    ...
    if (this.cartaSig > carta) {
      if (opcion === 'mayor') {
        ...
        // Solo restamos intentos si aún hay disponibles
        this.intentos--;
      }
    ...
    if (this.intentos <= 0) {
      ...
      this.obtenerYCrearResultado(false);
    }
    ...
    if (this.puntaje === 3 && this.numeroPartidasGanadas === 0) {
      ...
      this.reintentarJuego();
      this.obtenerYCrearResultado(true);
    }
  ...
  obtenerYCrearResultado(esGanador: boolean) {
    let fecha = new Date();
